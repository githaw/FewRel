...
from fewshot_re_kit.sentence_encoder import (
    ...
    BERTSentenceEncoder,
    CNNSentenceEncoder,
    ElectraPAIRSentenceEncoder,
    ElectraSentenceEncoder,
...
def main():
    ...
    parser.add_argument(
        "--encoder", default="bert", help="encoder: cnn or bert or roberta"
    )
    ...
    if encoder_name == "cnn":
        ...
        pretrain_ckpt = opt.pretrain_ckpt or "google/electra-small-discriminator"
        if opt.pair:
            sentence_encoder = ElectraPAIRSentenceEncoder(pretrain_ckpt, max_length)
        else:
            sentence_encoder = ElectraSentenceEncoder(
                pretrain_ckpt, max_length, cat_entity_rep=opt.cat_entity_rep
            )
    else:
